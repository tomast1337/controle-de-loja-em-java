...
public class Diversos {
	 ...
	 Double.parseDouble(dado); 
	   ...
	   mostrarDados("Houve erro na conversão \n"+
           "Digite apenas caracteres númericos"+erro.getMessage(), titulo, false);
            teste=false;
      }//Fim catchJFrmPublicacao.java:37
      return(teste);
  }//Fim do testa num
  ...
  public static String carNumerico(String texto, char car) {
    ...
    public static DefaultFormatterFactory FormatoCPF()
    {
        ...
        try
        {
            ...
            mostrarDados("CPF Formatada" + e.getMessage(), "Projeto", true);
        }
        DefaultFormatterFactory formato = new DefaultFormatterFactory(mascara, mascara);
        return (formato);
    }
    public static DefaultFormatterFactory FormatoTelefone(String titulo, int op){
        MaskFormatter mascara = null;
        try{
            switch(op) {
              case 0 : mascara = new MaskFormatter("(0##)####-####");
                           break;
              case 1 : mascara = new MaskFormatter("(0##)#####-####"); 
                          break;
            }
        }catch (ParseException e){
            mostrarDados("Telefone Formatada" + e.getMessage(), titulo, true);
        }
        DefaultFormatterFactory formato = new DefaultFormatterFactory(mascara, mascara);
        return (formato);
    }
